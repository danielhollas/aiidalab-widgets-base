...
class TrajectoryDataViewer(ipw.VBox):
    ...
    def __init__(self, trajectory, downloadable=True, **kwargs):
        ...
        self._structures = [
            trajectory.get_step_structure(i) for i in range(self._frames)
        ]
        ...
        self._plot_line = plt.plot(
            x_data,
            self._trajectory.get_array(self._property_selector.value)[: len(x_data)],
        )
        self._plot_circle = plt.scatter(
            x_data,
            self._trajectory.get_array(self._property_selector.value)[: len(x_data)],
        )
        ...
        self._plot_select_circle = plt.scatter(
            ...
            [
                self._trajectory.get_array(self._property_selector.value)[
                    self._step_selector.value
                ]
            ],
            stroke="red",
        )
        ...
        # Structure viewer.
        self._struct_viewer = StructureDataViewer(
            self._structures,
        ...
        children = [
            ipw.HBox(
                [
                    self._struct_viewer,
                    ipw.VBox(
                        [self._plot, self._property_selector], layout={"width": "50%"}
                    ),
                ]
            ),
            self._step_selector,
    ...
    def update_all(self, change=None):  # pylint: disable=unused-argument
        ...
        self._plot_circle.y = self._trajectory.get_array(self._property_selector.value)[
            : len(x_data)
        ]

        self._plot_line.x = x_data
        self._plot_line.y = self._trajectory.get_array(self._property_selector.value)[
            : len(x_data)
        ]
